...
def _clip_amplitude(amplitude: float) -> float:
    return min(float(amplitude), float(_channel().max_amp) * 0.85)


def _clip_detuning(detuning: float) -> float:
    max_detuning = float(_channel().max_abs_detuning) * 0.85
...
def summarize_register_physics(
    ...
    blockade_radius_um = round(device.rydberg_blockade_radius(_clip_amplitude(reference_amplitude)), 6)
    blockade_pairs = 0
    for row_index, row in enumerate(distance_matrix):
        for col_index, value in enumerate(row):
...
def build_sequence_from_candidate(
    ...
    amplitude = _clip_amplitude(sequence_candidate.amplitude)
    detuning = _clip_detuning(sequence_candidate.detuning)
    ...
    if family == "global_ramp":
        amplitude_start = _clip_amplitude(
            float(sequence_candidate.metadata.get("amplitude_start", max(0.0, amplitude * 0.15)))
        )
        ...
        ramp_pulse = Pulse.ConstantDetuning(
            RampWaveform(ramp_duration, _clip_amplitude(max(0.0, amplitude * 0.2)), amplitude),
            detuning,
            phase,
        )
        sweep_pulse = Pulse.ConstantAmplitude(
            amplitude,
            RampWaveform(hold_duration, detuning, _clip_detuning(detuning * 0.25)),
            phase,
        )
...
def build_simple_sequence_summary(
    ...
    payload: dict[str, Any] = {
        ...
        "controls": {
            "Omega_max": _clip_amplitude(sequence_candidate.amplitude) if PULSER_AVAILABLE else sequence_candidate.amplitude,
            "delta_start": _clip_detuning(sequence_candidate.detuning) if PULSER_AVAILABLE else sequence_candidate.detuning,
